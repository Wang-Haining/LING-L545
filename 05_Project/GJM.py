...
CHUNK_LENGTH = 20000  # 考虑到原文是在英文基础上做的，中英文互译比例，中:英=1.5-2:1，这里chunk的长度应该更长一些
INITIAL_FEATURE_SET_LENGTH_UNIGRAM = 100
INITIAL_FEATURE_SET_LENGTH_BIGRAM = 150

input = "/Users/hwang/Desktop/repo/Unmasking-JFZ/corpus"
output_gjm = "/Users/hwang/Desktop/repo/Unmasking-JFZ/gjm"
...
def sent_tokenizer_ch(text):
    ...
    slist = []
    for i in resentencesp.split(text):
...
def heavy_lifter(corpusdir, outputdir):
    # load training data
    jsonhandler_gjm.loadJson(corpusdir)
    ...
    for candidate in jsonhandler_gjm.candidates:
        database.add_author(candidate)  # database.authors = [cand1, cand2...]// database.texts = {author:[]...}
        for training in jsonhandler_gjm.trainings[candidate]:  # 对于某个candidate的training text的文件名
            # logging.info("Reading training text '%s' of '%s'", training, candidate)
            text = Text(jsonhandler_gjm.getTrainingText(candidate, training),  # 创建Text实例 text(raw, name)
    ...
    for unknown in jsonhandler_gjm.unknowns:
        ...
        unknown_text = Text(jsonhandler_gjm.getUnknownText(unknown), unknown)
            ...
            for known_text in database.texts[candidate]:
                ...
                for experiment in range(0, NUMBER_EXPERIMENTS):
                    ...
                    for i in range(0, NUMBER_ITERATIONS):
                        ...
                        print("进行", candidate, "的第" + str(experiment+1) + "个实验的第" + str(i+1) + "轮。", '\n')
    ...
    jsonhandler_gjm.storeJson(outputdir, results, dropped_features)
    # jsonhandler_gjm.storeJson(outputdir, results, )
