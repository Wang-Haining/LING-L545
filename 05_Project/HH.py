...
output_hh = "/Users/hwang/Desktop/repo/Unmasking-JFZ/hh"
...
def heavy_lifter(corpusdir, outputdir):
    # load training data
    jsonhandler_hh.loadJson(corpusdir)
    ...
    for candidate in jsonhandler_hh.candidates:
        database.add_author(candidate)  # database.authors = [cand1, cand2...]// database.texts = {author:[]...}
        for training in jsonhandler_hh.trainings[candidate]:  # 对于某个candidate的training text的文件名
            # logging.info("Reading training text '%s' of '%s'", training, candidate)
            text = Text(jsonhandler_hh.getTrainingText(candidate, training),  # 创建Text实例 text(raw, name)
    ...
    for unknown in jsonhandler_hh.unknowns:
        ...
        unknown_text = Text(jsonhandler_hh.getUnknownText(unknown), unknown)
    ...
    jsonhandler_hh.storeJson(outputdir, results, dropped_features)
    # jsonhandler_hh.storeJson(outputdir, results, )
