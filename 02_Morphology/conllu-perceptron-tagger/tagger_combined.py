...
try:
	...
	sys.path.append('/Users/hwang/Desktop/repo/LING-L545/02_Morphology')
	from perceptron import AveragedPerceptron
				...
				for words in sentence:
					...
					for i, token in enumerate(sentence):
						...
						if not tag:
							...
							features = self._get_features(i, token[1], context, prev, prev2, prev3)
							# make the prediction
...
#				print(c, n, '|||', sentence);
	...
	def _get_features(self, i, word, context, prev, prev2, prev3):
		...
		def glove_util(context, i, j):
		# """
		# Finds out the most similar word token to context[i]
		# """
			try:
				company = self.wv.most_similar(context[i])[j][0]
			except:
				company = 'OOV'
			return company
...
# cat ../UD_English-EWT/en_ewt-ud-train.conllu | python3 tagger.py -t en-ud.dat
# cat ../UD_English-EWT/en_ewt-ud-test.conllu | python3 tagger.py en-ud.dat > en-ud-test.out
# python3 ../evaluation_script/conll17_ud_eval.py --verbose ../UD_English-EWT/en_ewt-ud-test.conllu en-ud-test.out

# trainer(open('./02_Morphology/UD_English-EWT/en_ewt-ud-train.conllu') , 'aaa.dat')

# # this works
# t = PerceptronTagger('./02_Morphology/conllu-perceptron-tagger/en-ud.dat')
# t.tag(open('./02_Morphology/UD_English-EWT/en_ewt-ud-test.conllu'))
...
#
# # check if there is any out-of-dict token
# # for training set
# source_training = conllu.parse(open('./02_Morphology/UD_English-EWT/en_ewt-ud-train.conllu', "r").read())
#
# wordlist_training = []
# for item in source_training:
# 	wordlist_training.extend([a["form"] for a in item])
# wordlist_training = list(set(wordlist_training))
#
#
# word_not_in_model_training = [w for w in wordlist_training if not w.lower() in wv.index2word]
# # there are round 15% OOV words
# # first assign them random weights
# # try
#
